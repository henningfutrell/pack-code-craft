...
2. **This workflow's overlay wins over these defaults** — if
   `.agents/code-craft/code-craft-tdd.local.md` exists, read it; where it conflicts with this file, it
   wins.
...
- `code-craft-quality` — the architecture rules that make code testable in the first place
