...
instead — see `code-craft-quality`'s ratcheting section and its `references/ratchet.md`.
...
- **Testing legacy code usually requires a seam that does not exist yet.** Introducing it is
  an architecture change, not a test change (`code-craft-quality`). If the only way to test
