...
2. **This workflow's overlay wins over these defaults** — if
   `.agents/code-craft/code-craft-event-naming.local.md` exists, read it; where it conflicts with this
...
This skill reads `.agents/code-craft/code-craft-event-naming.local.md` on invocation if present. That
...
- `code-craft-quality` — domain events are part of its observability mandate (explicit, in
