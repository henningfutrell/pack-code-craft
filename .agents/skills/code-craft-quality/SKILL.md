...
2. **This workflow's overlay wins over these defaults** — if
   `.agents/code-craft/code-craft-quality.local.md` exists, read it; it wins on conflict. Records
...
- Tests written first and passing (see `code-craft-tdd`).

## Related

- `code-craft-tdd` — test-first; a CODE FLAG there is usually a boundary defect from here.
