...
- **The workflow's overlay** (`.agents/code-craft/code-craft-quality.local.md`) records only what
...
- **Extract on touch.** Every visit removes at least one coherent slice — the parent skill's
  rule. Cover the slice with characterization tests before moving it (`code-craft-tdd`, "Legacy
  substrate").
...
The legacy-test technique in `code-craft-tdd` is Michael Feathers' **characterization test**, from
